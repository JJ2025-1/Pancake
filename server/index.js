...
async function generateQuestion(topic, resource) {
    if (!process.env.OPENROUTER_API_KEY) {
        console.warn('OPENROUTER_API_KEY not found, using fallback.');
        const doc = nlp(topic);
        const nouns = doc.nouns().out('array');
        if (nouns.length > 0)
            return `What is the primary function of ${nouns[0]} in this context?`;
...
app.get('/user-data', (req, res) => {
    ...
    const streak = db.prepare('SELECT count, last_study_date FROM streaks WHERE username = ?').get('default_user');
...
app.get('/companion', async (req, res) => {
    ...
    try {
        ...
        if (state === 'focusing')
            prompt += "Give a very short (under 10 words) encouraging shout-out to someone currently focusing.";
        else if (state === 'completed')
            prompt += "Give a very short (under 10 words) congratulatory message for finishing a 30-minute focus session.";
        else
            prompt += "Give a very short (under 10 words) welcoming message to start focusing.";
        const result = await askAI(prompt);
